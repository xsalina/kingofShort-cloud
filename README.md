# 云开发 quickstart

这是云开发的快速启动指引，其中演示了如何上手使用云开发的三大基础能力：

- 数据库：一个既可在小程序前端操作，也能在云函数中读写的 JSON 文档型数据库
- 文件存储：在小程序前端直接上传/下载云端文件，在云开发控制台可视化管理
- 云函数：在云端运行的代码，微信私有协议天然鉴权，开发者只需编写业务逻辑代码

## 参考文档

- [云开发文档](https://developers.weixin.qq.com/miniprogram/dev/wxcloud/basis/getting-started.html)

[首页]
 ├─ 最新 10 条交易记录（卡片）
 ├─ 总收益 / 本月收益
 ├─ [查看更多] → 交易列表页（四 Tab）
 └─ [新增交易] → 添加交易页面

[交易列表页]
 ├─ Tab1：未卖出
 ├─ Tab2：挂单
 ├─ Tab3：部分卖
 ├─ Tab4：已卖
 └─ 点击卡片 → 卖出操作弹窗

[添加交易页面]
 ├─ 股票下拉选择
 ├─ 输入买入价格、数量、手续费、目标收益率
 ├─ 显示建议卖出价格 & 预期收益
 ├─ 按钮：挂单 / 跳过卖出
 └─ 可新增自定义股票

[我的页面]
 ├─ 交易 → 跳转交易列表页
 └─ 增加股票 → 跳转管理股票页面

[管理股票页面]
 ├─ 用户自定义股票列表
 ├─ 新增 / 编辑 / 删除
 └─ 默认股票不可删除

 短线有光，亦是幻象



 {
  _id: "tradeId_xxxx",

  userId: "userId_xxxx",
  stockId: "stockId_xxxx",

  // 股票快照
  stockName: "特斯拉",
  market: "美股",
  currency: "USD",                // ⭐ 必须有，用于按货币统计

  // 买入信息
  buyPrice: 100,
  buyQuantity: 10,
  buyFee: 1,
  buyTime: "2025-01-10T14:23:00",

  // 卖出记录
  sellRecords: [
    {
      sellId: "sellId_xxxx",
      price: 120,
      quantity: 5,
      fee: 0.5,
      sellTime: "2025-02-11T10:10:00",
      profit: 95,
      currency: "USD"            // ⭐ 卖出记录也应保存 currency
    }
  ],

  remainingQuantity: 5,
  status: "部分卖",

  // ⭐ 交易累积收益（同货币）
  totalProfit: 95,

  // ⭐ 最后一次卖出时间（做时间筛选）
  lastSellTime: "2025-02-11T10:10:00",

  createdAt: Date
}

| 字段                | 说明       |
| ----------------- | -------- |
| remainingQuantity | 剩余未卖出数量  |
| totalProfit       | 已卖出累计收益  |
| lastSellTime      | 最近一次卖出时间 |
| buyTime           | 买入时间     |
| 字段           | 说明     |
| ------------ | ------ |
| sellTime     | 卖出时间   |
| sellPrice    | 卖出价格   |
| sellQuantity | 卖出数量   |
| sellFee      | 卖出手续费  |
| avgCost      | 买入单股均价 |
| profit       | 该笔卖出收益 |



这个小程序用于管理股票买卖记录，帮助用户清晰掌握自己的持仓与收益情况。首页展示最新交易、总收益与本月收益，并支持一键新增交易记录。交易列表按“未卖出、挂单、部分卖、已卖”四类清晰分类，点击任意交易即可进行卖出操作。用户在添加交易时还能查看建议卖出价格和预期收益，并可自行维护自定义股票列表。

此外，点击收益区域会出现 金币掉落的趣味动画，让查看收益的过程更轻松、有成就感。